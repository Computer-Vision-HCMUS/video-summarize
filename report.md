...
- Sản phẩm đầu ra chính: **video tóm tắt động** \(V\_{\mathrm{sum}}\) có âm thanh liền mạch, được cắt ghép bằng **ffmpeg**.
...
- **Tóm tắt động (chính):** video \(V\_{\mathrm{sum}}\) có tổng độ dài khoảng **\(r \cdot |V|\)** với \(r\) cố định trong pipeline suy luận (trong triển khai báo cáo này \(r = 0{,}35\)), ghép từ các đoạn được chọn, **giữ nguyên âm thanh** tương ứng từng đoạn.
...
Việc gộp sigmoid vào loss dạng _logits_ giúp ổn định số học so với tính \(\sigma(z_t)\) rồi log riêng.
...
\[
e*t = \mathbf{u}^\top \tanh(\mathbf{W}\_a \mathbf{h}\_t + \mathbf{b}\_a),
\qquad
\alpha_t = \frac{\exp(e_t)}{\sum*{k=1}^{T}\exp(e_k)}.
\]

Vector ngữ cảnh toàn cục (pooling có trọng số):

\[
\mathbf{c} = \sum\_{t=1}^{T} \alpha_t \mathbf{h}\_t.
\]
...
7. **ffmpeg** cắt và nối các segment → file \(V\_{\mathrm{sum}}\) có đầy đủ audio.
...
Phần này mô tả ứng dụng **VideoIntel**, được thiết kế để nhận **video đã tóm tắt** (hoặc video bất kỳ) và trả lời câu hỏi: _khi thay video gốc bằng bản tóm tắt, các tác vụ phân tích nội dung (tìm kiếm, hỏi–đáp) còn giữ được chất lượng đến mức nào?_
...
**Trong benchmark đối chiếu:** dùng **một câu hỏi cố định** cho mọi video (tiếng Việt), ví dụ: _“Hãy mô tả nội dung tổng thể của video này dựa trên transcript. Trả lời bằng tiếng Việt, khoảng 3–5 câu.”_; giới hạn transcript đầu vào tương đương mức cắt trong ứng dụng; giới hạn độ dài sinh (ví dụ 512 token) để so sánh công bằng giữa gốc và tóm tắt.
...
### 9.7. Kết quả định tính Q&A

**Giao thức.** Cùng một **câu hỏi cố định** gửi cho Ollama (mô hình ví dụ llama3.2): *“Hãy mô tả nội dung tổng thể của video này dựa trên transcript. Trả lời bằng tiếng Việt, khoảng 3–5 câu.”* Transcript lấy từ Whisper trên **video gốc** và trên **video tóm tắt** tương ứng. Người đánh giá đối chiếu hai bản trả lời với nội dung thực tế của video gốc (và với nhau) rồi xếp loại: **Accurate** (khớp chủ đề và không có chi tiết sai), **Partial** (đúng hướng nhưng thiếu sót hoặc có chi tiết thêm/sai so với gốc), **Incorrect** (lệch chủ đề hoặc mô tả sai nội dung chính).

**Kết quả tổng hợp (10 video).** Khớp bảng tổng quan §9.6: trên **transcript video gốc**, **8/10** mẫu được đánh giá **đạt yêu cầu** (mức nội dung đủ tin cậy so với thực tế video). Trên **transcript video tóm tắt**, chỉ **5/10** mẫu giữ được mức tương đương (**Accurate**); **5/10** còn lại rơi vào **Partial** hoặc **Incorrect** — chủ yếu vì transcript **quá ngắn**, **thiếu đoạn**, hoặc bước tóm tắt chọn cảnh **lệch chủ đề**, khiến LLM thiếu bằng chứng hoặc **hallucination**.

**Bảng theo từng video (đối chiếu mô tả từ transcript gốc vs tóm tắt).**

| Video | Mức độ đánh giá (tóm tắt) | Kết quả định tính (nội dung so với video gốc) |
|-------|---------------------------|-----------------------------------------------|
| **video_01** | **Incorrect** | Gốc: tin Mỹ–Israel–Iran, lãnh đạo. Tóm tắt: mô tả **khác chủ đề hoàn toàn** (ví dụ thuốc kháng sinh / hài kịch) — lỗi nặng ở bước chọn nội dung tóm tắt. |
| **video_02** | **Accurate** | Gốc và tóm tắt đều nắm dòng tin Israel/Iran, năng lượng, xã hội Mỹ; bản tóm tắt còn mô tả được phong trào cực đoan tại HK — **khớp chủ đề**. |
| **video_03** | **Partial** | Gốc: bình luận bóng đá (nhiều đội/cầu thủ cụ thể). Tóm tắt: đúng hướng thể thao (FA Cup, Anh–Brazil) nhưng **thiếu** chi tiết đội/cầu thủ như bản gốc. |
| **video_04** | **Accurate** | Gốc và tóm tắt đều về kịch bản AI, rủi ro kiểm soát — **khớp**. |
| **video_05** | **Accurate** | Gốc: điện tử thần kinh, bệnh lý. Tóm tắt: BCI, vật liệu sinh học — **khớp**, thậm chí chi tiết hơn về mặt thuật ngữ. |
| **video_06** | **Partial** | Gốc: Trung Đông, Iran/Mỹ/Nga, dầu. Tóm tắt: đúng hướng địa chính trị nhưng **thêm** chi tiết “tai nạn giao thông Việt Nam” **không** có trong gốc (hallucination / nhiễu ngữ cảnh). |
| **video_07** | **Partial** | Gốc: Iran/Nga, Ukraine, năng lượng, EU, Slovakia. Tóm tắt: đúng phần kinh tế/TQ nhưng **thêm** “Việt Nam cạnh tranh kinh tế” **không** liên quan. |
| **video_08** | **Partial** | Gốc: Mỹ–Iran, biên giới, Khamenei. Tóm tắt: đúng Iran/Trump/TQ nhưng **thêm** bất động sản TQ, biểu tình NY — **lạc đề** phần phụ. |
| **video_09** | **Partial** | Gốc: nhiều sự kiện trong nước + quốc tế. Tóm tắt: phần quốc tế **gần đúng** nhưng **diễn giải sai** một số ý; **thiếu** phần nội dung trong nước — **nhất quán** với chỉ còn **~9,5%** số đoạn transcript so với gốc (bảng §9.6). |
| **video_10** | **Accurate** | Gốc và tóm tắt đều nắm hàng hải Trung Đông, an ninh, cứu nạn — **đủ chủ đề chính**. |

**Đọc kết quả.** Q&A trên tóm tắt **không** song song với chất lượng Search CLIP: khi transcript **mất mật độ đoạn** hoặc **sai chủ đề**, LLM vẫn sinh câu trả lời trôi chảy nhưng **không trung thành** video gốc; trường hợp **video_01** và **video_09** là hai cực (lệch chủ đề hoàn toàn vs thiếu hụt thông tin nặng).
...
6. B. Mahasseni, M. Lam, S. Todorovic, “Unsupervised Video Summarization with Adversarial LSTM Networks,” _IEEE Conference on Computer Vision and Pattern Recognition (CVPR)_, 2017 (**SUM-GAN**). URL: https://web.engr.oregonstate.edu/~sinisa/research/publications/cvpr17_summarization.pdf
7. K. Zhang, W.-L. Chao, F. Sha, K. Grauman, “Video Summarization with Long Short-Term Memory,” _European Conference on Computer Vision (ECCV)_, 2016. Trong cùng bài đề xuất hai biến thể **vsLSTM** và **dppLSTM** cho tóm tắt video.
8. K. Zhang, K. Grauman, F. Sha, “Retrospective Encoders for Video Summarization,” _AAAI Conference on Artificial Intelligence_, 2019 (**Cycle-SUM**). URL: https://ojs.aaai.org/index.php/AAAI/article/view/4948/4821
...
10. W. Zhu, J. Lu, J. Li, J. Zhou, “DSNet: A Flexible Detect-to-Summarize Network for Video Summarization,” _IEEE Transactions on Image Processing (TIP)_, 2021 (phiên bản preprint/OpenReview; có nhánh anchor-free). Repo: https://github.com/li-plus/DSNet
11. Y. Zhu, J. Li, L. Zhang, et al., “MHSCNet: Multi-Head Self-Attention Based Convolutional Network for Video Summarization,” _arXiv preprint_ arXiv:2204.08352, 2022. URL: https://arxiv.org/pdf/2204.08352
...
19. H. B. Haq, M. Asif, M. B. Ahmad, R. Ashraf, T. Mahmood, “Video summarization techniques: A review,” _Mathematical Problems in Engineering_, vol. 2021, pp. 1–17, 2021.
20. A. G. Money, H. Agius, “A survey on video summarization techniques,” _International Journal of Computer Applications_, vol. 118, no. 11, pp. 25–31, 2015.
21. T. Psallidas, P. Koromilas, T. Giannakopoulos, E. Spyrou, “Multimodal summarization of user-generated videos,” _Applied Sciences_, vol. 11, no. 11, p. 5260, 2021.
22. S. Hu, Z. Liu, J. Liu, Z. Guo, “Video summarization based on feature fusion and data augmentation,” _Computers_, vol. 12, no. 9, p. 186, 2023.
...
25. G. Bertasius, H. Wang, L. Torresani, “Is Space-Time Attention All You Need for Video Understanding?,” _Proc. Int. Conf. Machine Learning (ICML)_, 2021 (**TimeSformer**).
